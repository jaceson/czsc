...
# ========== 条件1优化参数（用于增加信号数量）==========
条件1优化模式 = True      # True=启用优化（放宽条件），False=使用原始严格条件
条件1大鱼阈值 = 25         # 条件1优化模式下的大鱼阈值（降低阈值增加信号，建议20-30）
条件1要求买入信号 = False  # 是否要求买入信号（True=要求ABC68，False=只要求鱼值，False信号更多）
条件1要求市场过滤 = False  # 是否要求市场过滤（False=不要求，增加信号）

# 条件1优化级别（可选，用于快速切换不同优化程度）
# '极宽松' - 阈值20，不要求买入信号，不要求市场过滤（信号最多）
# '宽松' - 阈值25，不要求买入信号，不要求市场过滤（当前默认）
# '中等' - 阈值30，要求买入信号，不要求市场过滤
# '严格' - 阈值35，要求买入信号，要求市场过滤
条件1优化级别 = '极宽松'  # 可选：'极宽松', '宽松', '中等', '严格'

# ========== 条件3优化参数（用于增加信号数量）==========
条件3优化模式 = True      # True=启用优化（放宽条件），False=使用原始严格条件
条件3涨幅跳跃阈值 = 5     # 条件3优化模式下的涨幅跳跃阈值（降低阈值增加信号，建议5-8）
条件3大鱼阈值 = 25        # 条件3中"昨天小鱼今大鱼"和"前天小鱼今大鱼"使用的大鱼阈值（降低阈值增加信号）

# 条件3优化级别（可选，用于快速切换不同优化程度）
# '极宽松' - 涨幅跳跃阈值3，大鱼阈值20（信号最多）
# '宽松' - 涨幅跳跃阈值5，大鱼阈值25（当前默认）
# '中等' - 涨幅跳跃阈值8，大鱼阈值30
# '严格' - 涨幅跳跃阈值10，大鱼阈值40（原始）
条件3优化级别 = '宽松'  # 可选：'极宽松', '宽松', '中等', '严格'
...
def get_bigfish_eat_smallfish_condition(symbol, df, debug_output='大鱼吃小鱼形态'):
    ...
    # 判断当前是否是大鱼嘴：根据优化模式调整条件
    if 条件1优化模式:
        # 根据优化级别自动设置参数
        if 条件1优化级别 == '极宽松':
            # 极宽松：阈值20，不要求买入信号，不要求市场过滤（信号最多）
            条件1使用的阈值 = 20
            条件1买入信号 = pd.Series([True] * len(df), index=df.index)
        elif 条件1优化级别 == '宽松':
            # 宽松：阈值25，不要求买入信号，不要求市场过滤（当前默认）
            条件1使用的阈值 = 25
            条件1买入信号 = pd.Series([True] * len(df), index=df.index)
            ...
            # 中等：阈值30，要求买入信号，不要求市场过滤
            条件1使用的阈值 = 30
            条件1买入信号 = ndf['ABC68'].fillna(False)
        elif 条件1优化级别 == '严格':
            # 严格：阈值35，要求买入信号，要求市场过滤
            条件1使用的阈值 = 35
            条件1买入信号 = ndf['ABC80'].fillna(False)
        else:
            # 使用手动设置的参数
            条件1使用的阈值 = 条件1大鱼阈值
            if 条件1要求买入信号:
                if 条件1要求市场过滤:
                    条件1买入信号 = ndf['ABC80'].fillna(False)  # 要求市场过滤
                else:
                    条件1买入信号 = ndf['ABC68'].fillna(False)  # 不要求市场过滤，只要求买入信号
            else:
                # 不要求买入信号，只要求鱼值达到阈值
                条件1买入信号 = pd.Series([True] * len(df), index=df.index)
    else:
        # 原始模式：使用原始阈值，要求ABC80（包含市场过滤）
        条件1使用的阈值 = 大鱼阈值
        条件1买入信号 = ndf['ABC80'].fillna(False)
    
    # 判断当前是否是大鱼嘴
    当前是大鱼嘴 = (当前鱼值 >= 条件1使用的阈值) & 条件1买入信号
    ...
    if 条件3优化模式:
        ...
        if 条件3优化级别 == '极宽松':
            ...
            条件3使用的涨幅跳跃阈值 = 涨幅跳跃阈值  # 使用原始值10
            条件3使用的大鱼阈值 = 大鱼阈值  # 使用原始值40
        else:
            # 使用手动设置的参数
            条件3使用的涨幅跳跃阈值 = 条件3涨幅跳跃阈值
            条件3使用的大鱼阈值 = 条件3大鱼阈值
    else:
        # 原始模式：使用原始阈值
        条件3使用的涨幅跳跃阈值 = 涨幅跳跃阈值
    ...
    涨幅跳跃大 = 涨幅跳跃 >= 条件3使用的涨幅跳跃阈值
    
    # 判断昨天或前天是小鱼，今天突然跳到大鱼（使用优化后的阈值）
    昨天小鱼今大鱼 = (当前鱼值_PREV1 < 小鱼阈值) & (当前鱼值 >= 条件3使用的大鱼阈值)
    前天小鱼今大鱼 = (当前鱼值_PREV2 < 小鱼阈值) & (当前鱼值_PREV1 < 小鱼阈值 * 1.2) & (当前鱼值 >= 条件3使用的大鱼阈值)
...
def main():
    ...
    print(f"  条件1优化模式: {条件1优化模式}")
    if 条件1优化模式:
        if 条件1优化级别 in ['极宽松', '宽松', '中等', '严格']:
            print(f"    条件1优化级别: {条件1优化级别}")
            if 条件1优化级别 == '极宽松':
                print(f"      阈值: 20, 不要求买入信号, 不要求市场过滤 (信号最多)")
            elif 条件1优化级别 == '宽松':
                print(f"      阈值: 25, 不要求买入信号, 不要求市场过滤 (当前默认)")
            elif 条件1优化级别 == '中等':
                print(f"      阈值: 30, 要求买入信号, 不要求市场过滤")
            elif 条件1优化级别 == '严格':
                print(f"      阈值: 35, 要求买入信号, 要求市场过滤")
        else:
            print(f"    条件1大鱼阈值: {条件1大鱼阈值} (原始: {大鱼阈值})")
            print(f"    条件1要求买入信号: {条件1要求买入信号}")
            print(f"    条件1要求市场过滤: {条件1要求市场过滤}")
    
    print(f"  条件3优化模式: {条件3优化模式}")
    if 条件3优化模式:
        if 条件3优化级别 in ['极宽松', '宽松', '中等', '严格']:
            print(f"    条件3优化级别: {条件3优化级别}")
            if 条件3优化级别 == '极宽松':
                print(f"      涨幅跳跃阈值: 3, 大鱼阈值: 20 (信号最多)")
            elif 条件3优化级别 == '宽松':
                print(f"      涨幅跳跃阈值: 5, 大鱼阈值: 25 (当前默认)")
            elif 条件3优化级别 == '中等':
                print(f"      涨幅跳跃阈值: 8, 大鱼阈值: 30")
            elif 条件3优化级别 == '严格':
                print(f"      涨幅跳跃阈值: {涨幅跳跃阈值}, 大鱼阈值: {大鱼阈值} (原始)")
        else:
            print(f"    条件3涨幅跳跃阈值: {条件3涨幅跳跃阈值} (原始: {涨幅跳跃阈值})")
            print(f"    条件3大鱼阈值: {条件3大鱼阈值} (原始: {大鱼阈值})")
