# coding: utf-8
"""
缠论二买 — Backtrader 回测实现

公式来源：CZSCStragegy_TwoBuy.py（手动回测版）
  1. 逐K线喂入CZSC，用 finished_bis 避免未来函数
  2. 检查缠论二买：中枢离开笔终点<当前下跌笔终点，前上涨笔终点<中枢上高
...
def calculate_twobuy_signal(df):
...
class TwoBuyStrategy(bt.Strategy):
    params = (
        ("hold_days", HOLD_DAYS),
        ("printlog", False),
    )
    ...
    def __init__(self):
        self.signal_line = self.datas[0].twobuy
...
def prepare_dataframe(df):
    signal = calculate_twobuy_signal(df)
    if signal is None:
        return None

    out = df[["open", "high", "low", "close", "volume"]].copy()
    out["twobuy"] = signal.fillna(False).astype(int)
...
def run_single_backtest(df_bt, cash=1000000, commission=0.0003):
    ...
    cerebro.addstrategy(TwoBuyStrategy, printlog=False)
...
def print_statistics(stats_dict):
    print()
    print("=" * 100)
    print("  缠论二买策略 — Backtrader 回测结果（全市场）")
    print("=" * 100)
...
if __name__ == "__main__":
    ...
    for symbol, df in all_data.groupby('code'):
        ...
        try:
            ...
            signal = calculate_twobuy_signal(df.reset_index(drop=True))
...
====================================================================================================
  ...
  --- 逐日收益明细 ---
      天数        均值%       中位数%        胜率%         总正         总负
  ------------------------------------------------------------
       1     +0.71%     +0.47%      60.7%   11061.40   -4416.90
       2     -0.51%     -0.46%      38.4%    2706.36   -4589.50
       3     -0.99%     -0.67%      35.9%    1367.14   -3606.14
       4     -1.99%     -1.52%      22.9%     569.40   -3460.95
       5     -2.42%     -1.75%      18.8%     396.65   -3114.75
====================================================================================================
'''