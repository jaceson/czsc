...
load_dotenv(dotenv_path=r"~\Desktop\ZB\git_repo\waditu\czsc\examples\test_offline\.env", verbose=True, override=True)
...
def test_fsa_base():
    ...
    # res = app.upload_file(file_path=r"~\Desktop\ZB\git_repo\zengbin93\fsa\fsa\spreed_sheets.py", parent_node='fldcnRHcnV1d1UDs7EWRk1PpcIh')
...
def test_im():
    ...
    image_key = app.upload_im_image(r"~\Desktop\Users\zengb\Downloads\十阶众生相.jpg")
    ...
    file_key = app.upload_im_file(r"~\Desktop\Users\zengb\Downloads\放量破年线.py")
    payload = {"receive_id": receive_id, "content": {"file_key": file_key}, "msg_type": "file"}
    print(app.send(payload))

    file_key = app.upload_im_file(r"~\Desktop\Users\zengb\Downloads\Think Python 2ed 中译版精校.pdf", file_type='pdf')
    payload = {"receive_id": receive_id, "content": {"file_key": file_key}, "msg_type": "file"}
    app.send(payload)

    app.send_text("快捷接口发送自定义文字", receive_id)
    app.send_image(r"~\Desktop\Users\zengb\Downloads\十阶众生相.jpg", receive_id)
    app.send_file(r"~\Desktop\Users\zengb\Downloads\Think Python 2ed 中译版精校.pdf", receive_id)
