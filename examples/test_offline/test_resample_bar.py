import sys
sys.path.insert(0, r"~\Desktop\ZB\git_repo\waditu\czsc")
import czsc
import pandas as pd
...
def test_check_freq_and_market():
    ...
    # gruop_name = "期货主力"
    gruop_name = "中证500成分股"
    files = Path(fr"~\Desktop\CZSC投研数据\{gruop_name}").glob("*.parquet")
    for file in files:
...
def get_future_times():
    files = Path(r"~\Desktop\CZSC投研数据\期货主力").glob("*.parquet")
    times = {}
    for file in files:
        df = pd.read_parquet(file)
    ...
    df = pd.read_excel(r"~\Desktop\test.xlsx")
