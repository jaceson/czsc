...
def run_opens_optim():
    ...
    results_path = r"~\Desktop\QMT投研\CCI入场优化结果B"
    files_position = [
        r"~\Desktop\QMT投研\基础策略V230707\A股日线CCI空头基准.json",
        r"~\Desktop\QMT投研\基础策略V230707\A股日线CCI多头基准.json",
    ]
    task_name = 'CCI入场优化'
...
def run_exits_optim():
    symbols = get_symbols('期货主力')[:10]
    results_path = r"~\Desktop\QMT投研\CCI出场优化结果B"
    files_position = [
        r"~\Desktop\QMT投研\基础策略V230707\A股日线CCI空头基准.json",
        r"~\Desktop\QMT投研\基础策略V230707\A股日线CCI多头基准.json",
    ]
