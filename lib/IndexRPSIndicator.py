...
from czsc_daily_util import read_json

class IndexRPSIndicator:
    """
    RPS（相对强度指标）计算器
    模拟通达信扩展数据的0-1000归一化顺序排名
    """
    
    def __init__(self, block_codes=None):
        """
        初始化RPS计算器
        ...
        if block_codes is None:
            ...
            print("正在从本地配置获取板块列表...")
            block_data = read_json('../data/block_list.json')
            for _, block_info in block_data.items():
                block_code = block_info['block_code']
                self.block_codes[block_code] = block_info['stocks']
            print(f"已加载 {len(self.block_codes)} 只股票")
        else:
    ...
    def fetch_price_data(self, days=250, start_date='2000-01-01'):
        """
        获取股票历史价格数据（从本地数据库）
